    document.addEventListener('DOMContentLoaded', function () {
      ...
      var budgetMinLabel = document.getElementById('budgetMinLabel');
      var budgetMaxLabel = document.getElementById('budgetMaxLabel');
      var budgetMinInput = document.getElementById('budgetMinInput');
      var budgetMaxInput = document.getElementById('budgetMaxInput');
      var ageRange = document.getElementById('ageRange');
      var ageMinLabel = document.getElementById('ageMinLabel');
      var ageMaxLabel = document.getElementById('ageMaxLabel');
      ...
      detailButtons.forEach(function (button, index) {
        button.addEventListener('click', function () {
          renderDetailPage(index % listingData.length);
        });
      });
      ...
      function initDualRange(root, minInput, maxInput, minLabel, maxLabel, formatter) {
        var track, fill, minThumb, maxThumb, dragging;
        var minLimit, maxLimit, step, minValue, maxValue;

        if (!root || !minInput || !maxInput || !minLabel || !maxLabel) {
          return;
        }

        track = root.querySelector('.dual-range-track');
        fill = root.querySelector('.dual-range-fill');
        minThumb = root.querySelector('.dual-range-thumb-min');
        maxThumb = root.querySelector('.dual-range-thumb-max');
        minLimit = Number(root.getAttribute('data-min'));
        maxLimit = Number(root.getAttribute('data-max'));
        step = Number(root.getAttribute('data-step')) || 1;
        minValue = Number(root.getAttribute('data-start-min'));
        maxValue = Number(root.getAttribute('data-start-max'));
        ...
        function updateUI() {
          var minPct = percent(minValue);
          var maxPct = percent(maxValue);
        ...
        function setFromClientX(clientX) {
          ...
          var nextValue = clamp(minLimit + ratio * (maxLimit - minLimit));
        ...
        minThumb.addEventListener('pointerdown', function (event) {
          event.preventDefault();
          minThumb.setPointerCapture(event.pointerId);
          startDrag('min', event.clientX);
        });

        maxThumb.addEventListener('pointerdown', function (event) {
          event.preventDefault();
          maxThumb.setPointerCapture(event.pointerId);
        ...
        root.addEventListener('click', function (event) {
          ...
          var clickedValue = clamp(minLimit + ratio * (maxLimit - minLimit));
          if (Math.abs(clickedValue - minValue) <= Math.abs(clickedValue - maxValue)) {
            minValue = Math.min(clickedValue, maxValue);
          } else {
            maxValue = Math.max(clickedValue, minValue);
          }
          updateUI();
        });
      ...
      initDualRange(budgetRange, budgetMinInput, budgetMaxInput, budgetMinLabel, budgetMaxLabel, function (value) {
        return '$' + value;
      });

      initDualRange(ageRange, ageMinInput, ageMaxInput, ageMinLabel, ageMaxLabel, function (value) {
        return String(value);
      });
